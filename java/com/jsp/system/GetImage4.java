...
@WebServlet(urlPatterns = { "/api/getImagePaths4" })
public class GetImage4 extends HttpServlet {

    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        int boardListNum4 = Integer.parseInt(req.getParameter("boardListNum4"));
        try (DBConnector con = new DBConnector();) {
            PostImageDao4 map = con.OpenMap(req, PostImageDao4.class);
            List<String> imagePaths4 = map.getImagePaths4(boardListNum4);
            resp.setContentType("application/json");
            resp.setCharacterEncoding("UTF-8");

            JSONObject obj = new JSONObject();
            JSONArray array = new JSONArray();
            obj.put("paths4", array);
            for(String path4 : imagePaths4) {
            	array.put(path4);
